...
import './VideoViewer.css' // Дополнительные стили для видео
...
function VideoViewer() {
  ...
  useEffect(() => {
    console.log('VideoViewer useEffect - location.state:', location.state)
    if (location.state?.fileData?.searchQuery) {
      setSearchQuery(location.state.fileData.searchQuery)
      handleSearch()
    }
  }, [location.state])
  ...
  // Функция для перехода к определенному времени в видео
  const seekToTime = (startTime) => {
    if (videoRef.current && startTime !== null && startTime !== undefined) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8001/api/v1/search/mp4_search', {
        method: 'POST',
        body: formData
      })
      ...
      addApiDataObject({
        "Total Results": data.sentences?.length || 0,
        "File Type": "Video (MP4)",
        "Query": searchQuery
      })
    ...
    <div className="pdf-viewer-app">
      <div className="pdf-viewer-container">
        ...
        <header className="pdf-header">
          ...
          <h1 className="pdf-title">Video Viewer</h1>
            ...
            <div className="file-info-container">
              ...
              <div className="file-details">
                ...
                <div className="detail-row">
                  ...
                  <span className="detail-value">Video File</span>
                </div>
              </div>
            </div>
              ...
              <div className="pdf-placeholder">
                <div className="placeholder-content">
                  ...
                  <p className="placeholder-note">
                    Video player will be displayed here when file is loaded
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="messages-list">
                {messages.length === 0 ? (
                  <div className="no-messages">
                    ...
                    <p className="no-messages-hint">Search the video to see results here</p>
                  </div>
                ) : (
